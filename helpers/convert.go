...
func parseValue(v interface{}, t string) interface{} {
	switch t {
	case "float64":
		if val, err := strconv.ParseFloat(string(v.([]byte)), 64); err == nil {
			return val
		} else {
			return nil
		}
...
func FlatResponse(data []map[string]interface{}, schema map[string]interface{}) []map[string]interface{} {
	...
	for _, d := range data {
		temp := make(map[string]interface{}, len(fields))
		for k, v := range d {
			if c, ok := fields[k]; ok {
				if v != nil {
					temp[c["as"]] = parseValue(v, c["type"])
				} else {
					temp[c["as"]] = v
				}
			}
